...
class create_agent(object):
    ...
    def __init__(self,batch_dim,env,network,nsteps,gamma):
    ...
    def train(self, max_iterations,stop_condition):
        ...
        while itr < max_iterations and not open_ai_solve_condition:   
            ...
            average_reward = np.mean(self.sum_eps_rwd[-100:])
            print("Mean Rewards: ", average_reward, " | Episode: ", self.count_episodes)
            average_reward_per_iteration.append(average_reward)        
            
            # Test if the env is already solved
            #open_ai_solve_condition = average_reward >= stop_condition

            itr += 1
            print("Iteration: ", itr)
        return average_reward_per_iteration
