...
# Reward to plot: shape((len(different_nsteps), iterations))
def plot_results(rewards_to_plot,different_nsteps,batch_dim):
        plt.figure(figsize=(12,8))
        for i in range(rewards_to_plot.shape[0]):
                plt.plot(rewards_to_plot[i], label='n-steps=' + str(different_nsteps[i]))
...
def main():
        ...
        # Try a renge of different N-Steps = [100, 20, 1, 10, 50, 30]
        different_nsteps = [100, 20, 1, 10, 50, 30]
        
        n_sim = 8

        ## Initialize Variables
        reward_to_plot = np.zeros((len(different_nsteps), iterations))
        env = gym.make(name_env)
        ...
        for ns in range(0,len(different_nsteps)):
                ...
                for i in range(0,n_sim):
                        ...
                        # *Core function* 
                        reward_to_plot[ns] += agent.train(iterations, open_ai_baseline)
        env.close()
        # Average data over simulations
        reward_to_plot /= n_sim
        plot_results(reward_to_plot,different_nsteps,batch_dim)
